...
def gen_data(n_qubits: int, num_seeds: int, num_steps: int, step_mes: int, f_name: str):
    ...
    for _gap in gaps:
        ...
        # If a file exists but only has 2 trajectory points (chunk_size=1000 old run), overwrite it with the chunk_size=10 run
        if os.path.exists(_ffname):
            try:
                ...
                if len(_d["sre"][0]) > 10:
                    ...
                    print(f"Overwriting coarse gap {n_qubits - _gap} file {_ffname} with chunk_size=10 run.")
            except Exception:
                pass
