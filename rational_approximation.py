...
def _(mo):
    ...
    This notebook imports optimization trajectory data, extracts the final states, filters out the ones that are **threshold Entangled (TE)**, and applies a rational approximation to their complex coefficients. It then checks if the resulting rationalized state remains TE.
    """)
    return
...
def _(combinations, np):
    ...
    def is_appt(x: np.ndarray) -> bool:
        _purity = np.real(np.sum(x * np.conj(x)))
        _D = x.shape[0]
        if _purity <= 1 / (_D - 1):
            return True
    ...
    def par_trace(psi, dim, n, n_parties):
        ...
        """
        n_rem = n - n_parties
        # Reshape state vector into a (dim^n_rem) x (dim^n_parties) matrix
        psi_mat = psi.reshape(dim**n_rem, dim**n_parties)
