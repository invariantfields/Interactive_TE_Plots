...
# Iterative Quantum State Transformation Step (run_entanglement_optimization)
def apply_entanglement_step_single(psi, eps=1e-10):
    ...
    for per, inv_per in zip(perms_list, inv_perms_list):
        ...
        rho_A = psi_perm @ psi_perm.conj().T
...
@jax.jit
def get_purity_jax(psi_batch):
    # Calculate purity for all states in batch
    def get_single_purity(psi):
        psi_tensor = psi.reshape((2,) * n_qubits)
        purities = []
        for per in perms_list:
            psi_perm = psi_tensor.transpose(per).reshape(rem_dim, k_dim)
            rho_A = psi_perm @ psi_perm.conj().T
            purities.append(jnp.sum(jnp.abs(rho_A)**2))
        return jnp.mean(jnp.array(purities))
    return vmap(get_single_purity)(psi_batch)
...
init_states = np.array([rand_Almost_Stab_state_np(n_qubits, 7) for _ in range(num_starts)])
psi_batch = jnp.array(init_states)

purity_0 = np.array(get_purity_jax(psi_batch))
sre_0 = np.array(_compute_sre_pure_jax_batch(psi_batch, H_jax, phases_jax, xor_indices_jax))
print(f"Step 0   | Mean SRE: {np.mean(sre_0):.4f} | Mean Purity: {np.mean(purity_0):.4f}")

for step in range(1, 251):
    psi_batch = vmapped_step(psi_batch)
    if step % 50 == 0:
        purity_val = np.array(get_purity_jax(psi_batch))
        sre_val = np.array(_compute_sre_pure_jax_batch(psi_batch, H_jax, phases_jax, xor_indices_jax))
        print(f"Step {step:3d} | Mean SRE: {np.mean(sre_val):.4f} | Mean Purity: {np.mean(purity_val):.4f}")
