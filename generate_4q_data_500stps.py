...
def run_jax_gpu_optimization(
    ...
    # Initialize JAX Solver with maxiter set to step_mes
...
def gen_data(n_qubits: int, num_seeds: int, num_steps: int, f_name: str):
    ...
    for _gap in gaps:
        ...
        run_jax_gpu_optimization(
            ...
            num_steps,
            n_qubits - _gap,
            step_mes=num_steps,  # Single chunk
            filename=_ffname,
        )
...
if __name__ == "__main__":
    ...
    # Parameters: 4 qubits, 1500 seeds, 500 steps (single chunk)
    stpsi = [500]
    qbt_nmbs = [4]
    num_seds = [1500]
    ...
    for idx in range(len(qbt_nmbs)):
        ...
        print(f"\n=======================================================")
        print(f"Running simulation for {qbt_nmbs[idx]} Qubits | {num_seds[idx]} Seeds | {stpsi[idx]} Steps (Single Chunk + Direct SRE)")
