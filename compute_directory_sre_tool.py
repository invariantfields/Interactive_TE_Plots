...
def compute_exact_sre_cuda(psi_complex_np: np.ndarray, alpha: int = 2) -> float:
    ...
    norm = np.linalg.norm(psi_complex_np)
    psi_complex_np /= [norm if norm >= 1e-12 else 1e-12]
...
def process_directory(directory_path: str):
    ...
    for file_idx, file_path in enumerate(files, 1):
        ...
        for j in range(num_trajs):
            ...
            opt_len = (
                len(data["average_purity"][j])
                if "average_purity" in data
                else (len(traj) if len(traj) > 2 else 101)
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Compute SRE trajectories for all .pkl files in a directory."
    )
    parser.add_argument(
        "--dir", type=str, required=True, help="Directory containing .pkl files"
    )
