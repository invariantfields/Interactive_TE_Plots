...
app._unparsable_cell(
    ...
    for st in range(num_starts):
        ...
        # KICKSTART: If gap is low, apply a random unitary to break product state trap
        if gap <= 2:
            psi = haar_random_unitary_gpu(2**n) @ psi
        ...
        for ent_step in range(num_loops):
            ...
            if ent_step % step_mes == 0:
                ...
                for j in combinations(range(n), k):
                    ...
                    x_m = par_trace(psi_moved, dim, n, k)
                    s.append(float(cp.sum(x_m * cp.conj(x_m)).real))
                    del x_m
...
app._unparsable_cell(
    ...
    for _gap in range(0, _n_qubits_test + 1):
            ...
            try:
                ...
                file_path = f"data/4_qbt_200_sds_ptmzng_fr_100_stps{_gap}.pkl"
                with open(file_path, "rb") as f:
                    loaded_data = pickle.load(f)

                # --- EXTRACT THE STATE ARRAY FROM THE DICTIONARY ---
                # 'final_states' contains a list of states per random start trajectory.
                # We grab the final state of the first trajectory ([0]) for our diagnostic test.
                if isinstance(loaded_data, dict) and "final_states" in loaded_data:
                    _psi_test = loaded_data["final_states"][0]
                else:
                    _psi_test = loaded_data

                # Push the pure complex numeric vector to the GPU backend safely
                _psi_test = cp.asarray(_psi_test, dtype=complex)
                # ---------------------------------------------------
    ...
    for _gap in range(0, _n_qubits_test + 1):
        ...
        try:
            ...
            with open(file_path, "rb") as f:
                loaded_data = pickle.load(f)
                print(loaded_data)
            # --- EXTRACT STATE FROM TRAJECTORIES DICTIONARY ---
            if isinstance(loaded_data, dict) and "final_states" in loaded_data:
                _psi_test = loaded_data["final_states"][0]
            else:
                _psi_test = loaded_data
    ...
    def render_diagnostics():
        return mo.vstack(
            [
                ...
                mo.accordion(
                    {"Detailed Diagnostic Diagnostic JSON Tree": mo.tree(diagnostic_results)}
                ),
            ]
        )
